...
export default class DB {
  constructor() {
    ...
    this[Symbol.for("indexedDB")] = window.indexedDB || window.mozIndexedDB || window.webkitIndexedDB || window.msIndexedDB;
    // DON'T use "var indexedDB = ..." if you're not in a function.
    // Moreover, you may need references to some window.IDB* objects:
    this[Symbol.for("IDBTransaction")] = window.IDBTransaction || window.webkitIDBTransaction || window.msIDBTransaction || {
      READ_WRITE: "readwrite"
    };
    // This line should only be needed if it is needed to support the object's constants for older browsers
    this[Symbol.for("IDBKeyRange")] = window.IDBKeyRange || window.webkitIDBKeyRange || window.msIDBKeyRange;
